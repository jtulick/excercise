...
input_variables = OutputFormatter.request_inputs
puts OutputFormatter.notification_confirmation(*input_variables)
...
text_files_in_directory.each do |text_file|
  ...
  parsed_files << parse_text(full_file_path, input_variables[1] \
    .downcase, input_variables[2].downcase)
end
...
if context_files_found.empty?
  ...
  puts OutputFormatter.results_formatter(context_files_found, *input_variables)
end
